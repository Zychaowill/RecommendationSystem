...
import com.jangz.recommendation.util.DBHelper;

public class BasicDao {
	/**
	 * 根据Sql和实体参数 查询数据库 返回结果集的第一个实体对象 1. setPreparedSql方法是对预置语句进行处理 2.
	 * getRsListFromMetaData方法是对结果集自动填充容器 3. 实体类里面需要调用的方法必须是对象类型 即int -> Integer
	 * 
	 * @param sql
	 * @param conn
	 * @param entity
	...
	/**
	 * 根据Sql和实体参数 查询数据库 返回结果集 1. setPreparedSql方法是对预置语句进行处理 2.
	 * getRsListFromMetaData方法是对结果集自动填充容器 3. 实体类里面需要调用的方法必须是对象类型 即int -> Integer
	 * 
	 * @param sql
	 * @param entity
	...
	/**
	 * 无where条件查询数据库 返回结果集 1. 最普通的查询不需要解析#{}里面的内容 2.
	 * getRsListFromMetaData方法是对结果集自动填充容器 3. 实体类里面需要调用的方法必须是对象类型 即int -> Integer
	 * 
	 * @param sql
	 * @param cls
	 ...
	 */
	private static List<?> getRsListFromMetaData(ResultSet rs, Class<?> cls) throws SQLException,
			InstantiationException, IllegalAccessException, ClassNotFoundException, SecurityException,
	 ...
	 * @param entity
	 */
	public static void setPreparedSql(String sql, PreparedStatement pstmt, Object entity)
	...
	/**
	 * 根据Sql和实体参数 对数据库进行 增,删,改 操作 setPreparedSql方法是对预置语句进行处理
	 * 
	 * @param sql
	 * @param entity
	 * @param conn
